...
def write_prepared_data(path: str) -> None:
    with open(path, "r") as file:
        ...
        with open(
            prepared_data_random_qber_filename, "w+", newline=""
        ) as random_qber_file, open(
            prepared_data_bursty_qber_filename, "w+", newline=""
        ) as bursty_qber_file:
...
def sort_and_prepare_data(filename: str) -> List[List[float]]:
    ...
    data.sort(
        key=lambda x: (
            x[ColumnsDataStats.L],
            x[ColumnsDataStats.QBER],
            x[ColumnsDataStats.N_K],
        )
    )  # sorted by L, QBER than N*K
    return data
